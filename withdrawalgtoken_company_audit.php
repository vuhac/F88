<?php
// ----------------------------------------------------------------------------
// Features:	後台-- 代幣(GTOEKN) 後台審查列表頁面
// File Name:	withdrawalgtoken_company_audit.php
// Author:		Barkley
// Log:
// ----------------------------------------------------------------------------
/*
...
// 初始化變數
// 功能標題，放在標題列及meta
// $tr['gtoken withdrawal application kanban'] = '現金取款申請看板';
$function_title 		= $tr['gtoken withdrawal application kanban'];
...
// 結果欄（右）title及內容
$paneltitle_content 	= '<span class="glyphicon glyphicon-list" aria-hidden="true"></span>'.$tr['Query results'].'<p id="tokenwithdrawal_mq" class="mb-0 ml-auto float-right" style="color: #dc3545; display: none;"></p>';
$panelbody_content		= '';
// ----------------------------------------------------------------------------
...
// 	// 表格欄位名稱
// 	// $tr['agent review seq'] = '申请单号';$tr['Account'] = '帳號'; $tr['agent review apply time'] = '申請的時間';$tr['agent review name'] = '姓名'; $tr['agent review phone'] = '電話'; $tr['IP/fingerprint'] = 'IP/指紋' $tr['withdrawal amount'] = '提款金額'; $tr['Fee'] = '手續費';$tr['administrative audit fee'] = '行政稽核費'; $tr['audit'] = '審覈';
	...
	<tr>
		...
		<th class="text-center">'.$tr['amount'].'</th>
		<th class="text-center">'.$tr['Fee'].'</th>
		<th class="text-center">'.$tr['administrative audit fee'].'</th>
		<th>'.$tr['apply_time'].'</th>
		<th>'.$tr['Payment method'].'</th>
		<th>'.$tr['ip address'].'</th>
		<th>'.$tr['audit'].'</th>
		<th class="text-center">'.$tr['detail'].'</th>
	</tr>
	';
...
/*
	...
	*/
	...
	// 提示管理人員如何進行操作。$tr['GTOKEN withdrawal application review'] = '代幣GTOKEN取款申請審核';
	$show_tips_html = '';
	//<div class="alert alert-success">
	// * ' .$tr['GTOKEN withdrawal application review'].'<br>
	// </div>
...
JS;
	...
	$(document).ready(function() {
		if (location.search == '?unaudit'){
			url = "withdrawapgtoken_company_audit_review_action.php?a=get_result&account=&agent=&sdate=2010-01-30%2000:00&edate="+moment().tz('America/St_Thomas').format('YYYY-MM-DD')+"%2359:59&amount_lower=&amount_upper=&ip=&transaction_id=&status_qy[]=2"
		} else{
			url = "withdrawapgtoken_company_audit_review_action.php?a=get_init"
		}
		$("#show_list").DataTable({
			"bProcessing": true,
			"bServerSide": true,
			...
			"columns":[
				...
				{"data":"amount"},
				{"data":"fee_amount"},
				{"data":"administrative_amount"},
				{"data":"applicationtime_tz"},
	...
	$("#submit_to_inquiry").click(function(){
		...
		$("#show_list").DataTable()
		.ajax.url("withdrawapgtoken_company_audit_review_action.php?a=get_result"+url_data)
		.load();
		// $.unblockUI();
