...
// 初始化變數
// 功能標題，放在標題列及meta
$function_title 		= $tr['member_withdrawalgcash'];
...
if($_SESSION['agent']->therole == 'R' ) {
	...
	'.$tr['member_withdrawalgcash text1'].' <br>
	'.$tr['member_withdrawalgcash text2'].$gcash_cashier_account.' '.$tr['Account'].'。<br>
	'.$tr['member_withdrawalgcash text3'].'
	</div>';
	...
	if($_SESSION['agent']->therole == 'R') {
		...
		if($r[0] == 1) {
			...
			$source_transferaccount_result_html 	= $tr['Account exists'].'，'.$tr['Balance'].' '.money_format('%i', $r[1]->gcash_balance);
		}else{
			$source_transferaccount_input_default = $_SESSION['agent']->account;
