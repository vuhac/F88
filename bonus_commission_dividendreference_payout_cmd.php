...
<?php
// ----------------------------------------------------------------------------
// Features:	後台-- 放射線組織加盟金計算 -- 股利發放 -- 獨立排程執行
// File Name:	bonus_commission_dividendreference_payout_cmd.php
// Author:		Ian
// Related:	bonus_commission_dividendreference.php
// 					bonus_commission_dividendreference_action.php
// 				DB table:  root_receivemoney 彩金發放
// 				DB table:  root_dividendreference
// 				DB table:  root_dividendreference_setting
// Log:
// ----------------------------------------------------------------------------
// How to run ?
// usage command line : /usr/bin/php70 bonus_commission_dividendreference_payout_cmd.php test/run setting_id statustoken worker
// ----------------------------------------------------------------------------
...
if(isset($argv[1]) AND ($argv[1] == 'test' OR $argv[1] == 'run') ){
  if(isset($argv[2])){
		...
		$dividendreference_setting = filter_var($argv[2], FILTER_VALIDATE_INT);
  }else{
		// command 動作 時間
		echo "command [test|run] setting_id statustoken worker \n";
		die('no setting_id');
	}
	if(isset($argv[3])){
	  $bonusstatus = get_object_vars(jwtdec('dividendpayout',$argv[3]));
	}else{
	  // command 動作 時間
	  echo "command [test|run] setting_id statustoken worker \n";
	  die('no statustoken');
...
// ----------------------------------------------------------------------------
if($web_check == 1){
	$output_html  = '<p align="center">round 1. 發放會員股利 - 更新中...<img src="ui/loading.gif" /></p><script>setTimeout(function(){location.reload()},500);</script>';
	file_put_contents($reload_file,$output_html);
}elseif($web_check == 2){
	$updatlog_note = 'round 1. 發放會員股利 - 更新中';
...
// -----------------------------------------------------------------------
//
$userlist_sql = "SELECT dailydate_start,dailydate_end FROM root_dividendreference_setting
WHERE id = '".$dividendreference_setting."' ORDER BY id;";
$dividend_setting = runSQLall($userlist_sql);

$b['dailydate_start'] = $dividend_setting[1]->dailydate_start;
$b['dailydate_end'] = $dividend_setting[1]->dailydate_end;
$b['daterange'] = preg_replace('/([^A-Za-z0-9])/ui', '',$b['dailydate_start']).'至'.preg_replace('/([^A-Za-z0-9])/ui', '',$b['dailydate_end']);

// 算 root_member 人數
$userlist_sql = "SELECT id,memberid,member_account,member_dividend_assigned FROM root_dividendreference
...
if($userlist[0] >= 1) {
	...
	for($i = 1 ; $i <= $userlist[0]; $i++){
		...
		if($b['member_dividend_assigned'] > 0){
		...
		// 更新 分紅 記錄
		$update_sql = "UPDATE root_dividendreference SET updatetime = NOW(),note = '已匯出至出納' WHERE id = '".$userlist[$i]->id."';";

		if($debug == 1) {
			echo '日期'.$current_date.'update會員股利'.$b['member_account'].' 更新SQL=';
			print_r($update_sql);
		}
	...
	$dividendreference_setting_update_sql = 'UPDATE "root_dividendreference_setting" SET "updatetime" = NOW(),"note" = \''.$current_date.':已匯出至出納\' WHERE id = \''.$dividendreference_setting.'\';';
	//echo $dividendreference_setting_update_sql;
	$dividendreference_setting_update_result = runSQL($dividendreference_setting_update_sql);
}
