...
<?php
// ----------------------------------------------------------------------------
// Features:	後台-- 放射線組織加盟金計算 -- 營業獎金發放 -- 獨立排程執行
// File Name:	bonus_commission_sale_payout_cmd.php
// Author:		Ian
...
// ----------------------------------------------------------------------------
// How to run ?
// usage command line : /usr/bin/php70 bonus_commission_sale_payout_cmd.php test/run 2017-06-06 statustoken worker
// ----------------------------------------------------------------------------
...
if(isset($argv[1]) AND ($argv[1] == 'test' OR $argv[1] == 'run') ){
  if(isset($argv[2]) AND validateDate($argv[2], 'Y-m-d') ){
		...
		$current_datepicker = date( "Y-m-d", strtotime( "$current_datepicker_start + $stats_bonus_days day"));
  }else{
		// command 動作 時間
	...
	if(isset($argv[3])){
	  $bonusstatus = get_object_vars(jwtdec('salebonuspayout',$argv[3]));
	}else{
	  // command 動作 時間
	  echo "command [test|run] YYYY-MM-DD statustoken worker \n";
	  die('no statustoken');
...
if(isset($argv[5]) AND $argv[5] == 'web' ){
	...
	$file_key = sha1('salebonuspayout'.$current_datepicker_start);
...
// ----------------------------------------------------------------------------
if($web_check == 1){
	$output_html  = '<p align="center">round 1. 發放會員分紅 - 更新中...<img src="ui/loading.gif" /></p><script>setTimeout(function(){location.reload()},500);</script>';
	file_put_contents($reload_file,$output_html);
}elseif($web_check == 2){
	$updatlog_note = 'round 1. 發放會員分紅 - 更新中';
...
WHERE dailydate_start = '$current_datepicker_start' AND dailydate_end = '$current_datepicker' AND member_bonusamount_paid IS NULL ORDER BY member_id ASC;";
// 取出 root_member 資料
...
$b['daterange'] = preg_replace('/([^A-Za-z0-9])/ui', '',$current_datepicker_start).'至'.preg_replace('/([^A-Za-z0-9])/ui', '',$current_datepicker);
...
if($userlist[0] >= 1) {
	...
	for($i = 1 ; $i <= $userlist[0]; $i++){
		...
		$prizecategories = $b['daterange'].'期間分紅';
		...
		if($debug == 1) {
			echo '日期'.$current_date.'update會員分紅'.$b['member_account'].' 更新SQL=';
			print_r($update_sql);
		}
