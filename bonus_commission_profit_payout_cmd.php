...
<?php
// ----------------------------------------------------------------------------
// Features:	後台-- 放射線組織加盟金計算 -- 營運利潤獎金發放 -- 獨立排程執行
// File Name:	bonus_commission_profit_payout_cmd.php
// Author:		Ian
// Related:	bonus_commission_profit.php
// 					bonus_commission_profit_action.php
// 				DB table:  root_receivemoney 彩金發放
// 				DB table:  root_statisticsbonusprofit 營運獎金報表, 資料由每日統計報表生成, 並且計算後輸出到此表.
// Log:
// ----------------------------------------------------------------------------
// How to run ?
// usage command line : /usr/bin/php70 bonus_commission_profit_payout_cmd.php test/run 2017-06-06 statustoken worker
// ----------------------------------------------------------------------------
...
if(isset($argv[1]) AND ($argv[1] == 'test' OR $argv[1] == 'run') ){
  if(isset($argv[2]) AND validateDate($argv[2], 'Y-m-d') ){
	...
	if(isset($argv[3])){
	  $bonusstatus = get_object_vars(jwtdec('profitbonuspayout',$argv[3]));
	}else{
	  // command 動作 時間
	  echo "command [test|run] YYYY-MM-DD statustoken worker \n";
	  die('no statustoken');
...
if(isset($argv[5]) AND $argv[5] == 'web' ){
	...
	$file_key = sha1('profitbonuspayout'.$current_datepicker);
...
// ----------------------------------------------------------------------------
if($web_check == 1){
	$output_html  = '<p align="center">round 1. 發放會員營運利潤獎金 - 更新中...<img src="ui/loading.gif" /></p><script>setTimeout(function(){location.reload()},500);</script>';
	file_put_contents($reload_file,$output_html);
}elseif($web_check == 2){
	$updatlog_note = 'round 1. 發放會員營運利潤獎金 - 更新中';
...
WHERE dailydate_start = '$current_datepicker' AND member_profitamount != '0' AND member_profitamount_paidtime IS NULL ORDER BY member_id ASC;";
// 取出 root_member 資料
...
if($userlist[0] >= 1) {
	...
	for($i = 1 ; $i <= $userlist[0]; $i++){
		...
		$profit_payout = $b['member_profitamount'] - abs($b['lasttime_stayindebt']);
		...
		$update_sql = "UPDATE root_statisticsbonusprofit SET
		 updatetime = now(),
		 member_profitamount_paid = '".$profit_payout."',
		 member_profitamount_paidtime = now()
		 WHERE id = '".$b['id']."';";

		if($debug == 1) {
			echo '日期'.$current_date.'update會員營運利潤獎金'.$b['member_account'].' 更新SQL=';
			print_r($update_sql);
		}
