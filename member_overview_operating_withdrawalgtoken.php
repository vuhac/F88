...
if($_SESSION['agent']->therole == 'R' ) {
	...
	* 人工游戏币提款（GTOKEN）功能：此为管理员或允许的客服人员进行人工（GTOKEN）提出的功能<br>
	* 此功能无须进行提款稽核，主要提供为管理员或客服进行反水或优惠额度发送的游戏币回收操作。 <br>
	* 游戏币领出就是会员的游戏币转帐到 '.$gtoken_cashier_account.' 帐号。<br>
	* 如果是「实际存提」，则表示此为真实存取行为，提供管理员注记使用。
	</div></div>';
	...
	if($_SESSION['agent']->therole == 'R') {
		...
		if($r[0] == 1) {
			...
			$source_transferaccount_result_html 	= '帳號存在，餘額 '.money_format('%i', $r[1]->gtoken_balance);
		}else{
			$source_transferaccount_input_default = $_SESSION['agent']->account;
	...
	<div class="form-group row">
		...
		<div class="col-sm-7">
			<input type="text" class="form-control" id="source_transferaccount_input" placeholder="" value="'.$source_transferaccount_input_default.'" '.$disabled_var.' disabled>
		</div>
        <div class="col-sm-3">
            <p class="mb-0" id="source_transferaccount_result">'.$source_transferaccount_result_html.'</p>
        </div>
	</div>
	';
	...
	$body_content		= $body_content.'
	<div class="form-group row">
		<label class="col-sm-2 col-form-label"><span class="text-danger" aria-hidden="true">*</span>提款金额</label>
		<div class="col-sm-7">
	...
	<div class="form-group row">
		...
		<div class="col-sm-7">
			...
			<p class="mb-0 text-secondary"><span class="text-danger">*</span> 代币提款行政费费用，设计为人工收取行政稽核不通过费用。</p>			
		</div>
		<div class="col-sm-3">
            <div id="realcash_desc" class="form-check">
                <input class="form-check-input" type="checkbox" id="realcash_input" name="realcash_input" value="1">
                <label class="form-check-label" for="realcash_input">實際存提</label>
            </div>
		</div>
	</div>
	';
...
HTML;
	...
	<div class="form-group row">
		...
		<div class="col-sm-7">
			<textarea class="form-control" id="system_note_input" placeholder="备注或是说明"></textarea>
		</div>
	...
	$body_content		= $body_content.'
	<div class="form-group row">
		<label class="col-sm-2 col-form-label"><span class="text-danger" aria-hidden="true">*</span>来源帐号的密码</label>
		<div class="col-sm-7">
...
<ul class="nav nav-tabs mt-3" id="memberoverviewTab" role="tablist">
  ...
  <li class="nav-item">
    <a class="nav-link active" id="transactionrecord-tab" href="#" role="tab" aria-controls="transactionrecord" aria-selected="false">
...
<div class="tab-content tab_p_overview" id="overviewoperating">
    <div class="row my-3">
        <div class="col-12 col-md-8 mx-auto">
            ...
            <div class="tab-pane fade show active" id="transactionrecord" role="tabpanel" aria-labelledby="transactionrecord-tab">
                <!-- 提出遊戲幣 -->	
                    {$panelbody_content}
            </div>
