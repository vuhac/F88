...
// ----------------------------------------------------------------------------
// Features: 子網域管理詳細
// File Name:	subdomain_management_detail.php
// Author: Neil
// Related:
// Log:
// ----------------------------------------------------------------------------
...
// 功能標題，放在標題列及meta
$function_title 		= $tr['Edit subdomain'];//'编辑子网域'
...
function combineWebTypeHtml($type)
{
  $option = '';
  ...
  foreach ($webType as $v) {
    $isSelected = ($type == $v) ? 'selected' : '';

    $option .= <<<HTML
    <option {$isSelected}>{$v}</option>
HTML;
  }
...
// function combineThemePathHtml($id, $themeSetting = '')
// {
//   $option = '';
...
//     $isSelected = (!empty($themeSetting) && $v->name == $themeSetting) ? 'selected' : '';
//     $option .= <<<HTML
//     <option {$isSelected}>{$v->name}</option>
// HTML;
//   }
...
$subDomainSetting = json_decode($domainSetting->configdata);

if (!in_array($sid, array_keys((array)$subDomainSetting))) {
  echo "<script>alert('".$tr['This domain setting could not be found']."');location.href='./subdomain_management.php'</script>";
}

// $cardHtml = combineCardHtml($subDomainSetting->$sid->style);
//手機桌機樣板選擇
$flatformData=$subDomainSetting->$sid->style;
$cardHtml = '';
$flatformStr = [
...
foreach ($flatformData as $flatform => $flatformSetting) {
  // $themePathHtml = combineThemePathHtml($flatform, $flatformSetting->themepath);
  $themeList[$flatform] = themeList($flatform,$flatformSetting->themepath);
  $cardHtml .= <<<HTML
  <div class="col-sm-6">
    <div class="card">
      <div class="card-header">{$flatformStr[$flatform]}</div>
      <div class="card-body">
        <form>
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              {$themeList[$flatform]['html']}
            </div>
          </div>
          <div class="form-group row">
            <label for="{$flatform}SubadmainName" class="col-sm-2 col-form-label">{$tr['SubadmainName']}</label>
            <div class="col-sm-10">
              <input type="text" class="form-control" id="{$flatform}SubadmainName" placeholder="{$tr['SubadmainName']}" value="{$flatformSetting->suburl}">
            </div>
          </div>
        </form>
      </div>
...
$webTypeHtml = combineWebTypeHtml($subDomainSetting->$sid->website_type);

$domainIsOpen = ($domainSetting->open == 1) ? 'checked' : '';
$subDomainIsOpen = ($subDomainSetting->$sid->open == 1) ? 'checked' : '';

//網域廣告設定
if(isset($subDomainSetting->$sid->component)){
  $component_id = $subDomainSetting->$sid->component;
}
else{
  $component_id = 0;
}
...
<form>
  <div class="form-group row">
    ...
    <div class="col-sm-4">
      <input type="text" class="form-control" id="admainUrl" placeholder="{$tr['domain']}" value="{$domainSetting->domainname}">
    </div>
  </div>
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <div class="col-12 material-switch pull-left">
        <input class="form-check-input switch" id="admainStatus" type="checkbox" {$domainIsOpen}>
...
$companyName = $subDomainSetting->$sid->companyName??'';
$companyShortName = $subDomainSetting->$sid->companyShortName??'';
$websiteName = $subDomainSetting->$sid->websiteName??'';
$companyLogo = $subDomainSetting->$sid->companylogo??'';
$companyFavicon = $subDomainSetting->$sid->companyFavicon??'';
$html .= <<<HTML
<div class="card">
  <div class="card-header">{$tr['subdomain site']}</div>
  <div class="card-body">
    <form>
      <div class="form-group row">
        ...
        <div class="col-sm-4">
          <input type="text" class="form-control" id="websiteName" placeholder="{$tr['subdmoain name']}" value="{$websiteName}">
        </div>
      </div>
      <div class="form-group row">
        <label for="websiteFooter" class="col-sm-2 col-form-label">{$tr['websiteFooter']}</label>
        <div class="col-sm-4">
          <input type="text" class="form-control" id="websiteFooter" placeholder="{$tr['websiteFooter']}" value="{$subDomainSetting->$sid->footer}">
        </div>
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-4">
          <input type="text" class="form-control" id="hostName" placeholder="{{$tr['hostName']}}" value="{$subDomainSetting->$sid->hostname}">
        </div>
        <label for="googleID" class="col-sm-2 col-form-label">{$tr['googleID']}</label>
        <div class="col-sm-4">
          <input type="text" class="form-control" id="googleID" placeholder="{$tr['googleID']}" value="{$subDomainSetting->$sid->google_analytics_id}">
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <div class="col-12 material-switch pull-left">
            <input class="form-check-input switch" id="subadmainStatus" type="checkbox" {$subDomainIsOpen}>
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-4">
          <input type="text" class="form-control" id="agent" placeholder="{$tr['Identity Agent']}" value="{$subDomainSetting->$sid->default_agent}">
        </div>
      </div>
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-4">
          <input type="text" class="form-control" id="companyName" placeholder="{$tr['companyName']}" value="{$companyName}">
        </div>
        <label for="companyShortName" class="col-sm-2 col-form-label">{$tr['companyShortName']}</label>
        <div class="col-sm-4">
          <input type="text" class="form-control" id="companyShortName" placeholder="{$tr['companyShortName']}" value="{$companyShortName}">
        </div>
      </div>
      ...
      <div class="form-group row">
        <label for="company-logo" class="col-sm-2 col-form-label">logo
          ...
          <img id="companylogo" style="max-height: 200px;max-width: 200px;" onerror="this.src='in/component/common/error.png'" src="{$companyLogo}">
        </div>
				<div class="col-xs-6">
      ...
      <div class="form-group row">
        <label for="company-logo" class="col-sm-2 col-form-label">favicon
          <span class="glyphicon glyphicon-info-sign mr-2" data-toggle="tooltip" data-placement="top"  title="{$tr['Image file does not exceed 2MB']}">
				...
				<div class="col-auto pl-0">
          <img id="companylogo" style="max-height: 50px;max-width: 50px;" onerror="this.src='in/component/common/error.png'" src="{$companyFavicon}">
        </div>
				<div class="col-xs-6">
					<div class="row border py-3 m-2 m-md-0">
						<div class="col-auto">							  
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <textarea class="form-control" id="note" rows="5">{$domainSetting->note}</textarea>
        </div>
      </div>
    </form>
    <div class="row">
...
$desktopThemePath = ($themeList['desktop']['status']=="custom")? '\''.$subDomainSetting->$sid->style->desktop->themepath.'\'':'$(\'#desktopThemePath\').val()';

$mobileThemePath = ($themeList['mobile']['status']=="custom")? '\''.$subDomainSetting->$sid->style->mobile->themepath.'\'':'$(\'#mobileThemePath\').val()';

$extend_js = <<<JS
<script>
$(document).on("click",'#submitSetting',function(){
  var logo_img=$('#upload_logo_img')[0].files[0]?$('#upload_logo_img')[0].files[0]:$('#logo_url').val()
  if(logo_img=='') logo_img='{$companyLogo}'

  var favicon_img=$('#upload_favicon_img')[0].files[0]?$('#upload_favicon_img')[0].files[0]:$('#favicon_url').val()
  if(favicon_img=='') favicon_img='{$companyFavicon}'
  ...
  var data = {
    ...
    'agent' : $('#agent').val(),
    'note' : $('#note').val(),
    'mobileThemePath' : {$mobileThemePath},
    'mobileSubadmainName' : $('#mobileSubadmainName').val(),
    'desktopThemePath' : {$desktopThemePath},
