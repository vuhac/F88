...
if(isset($_GET['a'])) {
  ...
  // 不合法的測試, 轉回 home.php. function 定義在 lib.php : todo
...
function memberaccount_create_check($account_create_input) {
    ...
    //var_dump($matches);
      ...
      // $tr['The account is not valid, the account can only be a-z 0-9 and other words. And the first letter must be English letters. Length of 3 to 12 characters'] = '帳號不合法，帳號只能為 a-z 0-9 等文字組成。且第一個字母需為英文字母。長度 3~12 個字元。';
			$account_check_return['text'] = '<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span>'.$tr['The account is not valid, the account can only be a-z 0-9 and other words. And the first letter must be English letters. Length of 3 to 12 characters'].'</div>';
...
// var_dump($_POST);
  ...
  AND (isset($_POST['account_create_input']) AND $_POST['account_create_input'] != NULL)
  // AND (isset($_POST['agent_account_input']) AND $_POST['agent_account_input'] != NULL)
  // AND (isset($_POST['bankname_input']) AND $_POST['bankname_input'] != NULL)
  // AND (isset($_POST['bankaccount_input']) AND $_POST['bankaccount_input'] != NULL)
  // AND (isset($_POST['bankprovince_input']) AND $_POST['bankprovince_input'] != NULL)
  // AND (isset($_POST['bankcounty_input']) AND $_POST['bankcounty_input'] != NULL)
  ) {
		// var_dump($_POST);
    // ------------------
		// echo '建立帳號';
		// ------------------
		$check_memberaccount = memberaccount_create_check($_POST['account_create_input']);

    if (isset($_POST['lv_one_agent_select']) AND filter_var($_POST['lv_one_agent_select'], FILTER_SANITIZE_STRING) == 'true') {
      $check_agentaccount['account'] = $config['system_company_account'];
      $check_agentaccount['id'] = $config['system_company_id'];
      $check_agentaccount['code'] = 1;
    } else {
      ...
      $user['commissionrule'] = filter_var($_POST['commission_select'], FILTER_SANITIZE_STRING);

      $user['bankname_input'] = (filter_var($_POST['bankname_input'], FILTER_SANITIZE_STRING) != '') ? filter_var($_POST['bankname_input'], FILTER_SANITIZE_STRING) : NULL;
      $user['bankaccount_input'] = (filter_var($_POST['bankaccount_input'], FILTER_SANITIZE_STRING) != '') ? filter_var($_POST['bankaccount_input'], FILTER_SANITIZE_STRING) : NULL;
      $user['bankprovince_input'] = (filter_var($_POST['bankprovince_input'], FILTER_SANITIZE_STRING) != '') ? filter_var($_POST['bankprovince_input'], FILTER_SANITIZE_STRING) : NULL;
      $user['bankcounty_input'] = (filter_var($_POST['bankcounty_input'], FILTER_SANITIZE_STRING) != '') ? filter_var($_POST['bankcounty_input'], FILTER_SANITIZE_STRING) : NULL;

      $user['recommendedcode'] = sha1($user['memberaccount']);

      if ($user['grade'] != '' AND $user['favorablerule'] != '') {

        $sql = 'INSERT INTO "root_member" ("account", "nickname", "realname", "passwd", "changetime", "creditcurrency", "mobilenumber", "email", "lang", "status"
          , "therole", "parent_id", "notes", "sex", "birthday", "wechat", "qq"
          , "bankaccount", "bankname", "bankprovince", "bankcounty", "withdrawalspassword", "timezone", "favorablerule"
          , "grade", "enrollmentdate", "recommendedcode", "becomeagentdate", "commissionrule")';
        $sql = $sql." VALUES ('".$user['memberaccount']."', NULL, '".$user['realname_input']."', '".$user['default_password']."', 'now()', '" .$config['currency_sign']."', '".$user['mobilenumber_input']."', '".$user['email_input']."', '".$user['lang']."', '".$user['status']."'
        , '".$user['therole']."', '".$user['parent_id']."', '".$user['notes_input']."', '".$user['sex_input']."', '".$user['birthday_input']."', '".$user['wechat_input']."', '".$user['qq_input']."'
        , '".$user['bankname_input']."', '".$user['bankaccount_input']."', '".$user['bankprovince_input']."', '".$user['bankcounty_input']."', '".$user['withdrawalspassword']."', '".$user['timezone']."', '".$user['favorablerule']."'
        , '".$user['grade']."', 'now()', '".$user['recommendedcode']."', now(), '".$user['commissionrule']."');";
        ...
        if($insertresult == 1) {
          ...
          if($user_result[0] == 1) {
            ...
            if($wallets_result == 1) {
              $r['code'] = '1';
                //管理員建立, 帳號完成
              $r['messages'] = $tr['Administrator established'].' '.$user['memberaccount'].' '.$tr['Account is completed'].' Default password is ['.$default_password.']';
            } else {
              $r['code'] = '2';
                //建立會員時的錢包出錯，請聯絡開發人員。
        ...
        $r['code'] = '5';
        // $tr['Please select the correct agent level, membership level, bouns level, commission set'] = '請選擇正確代理等級、會員等級、反水等級、傭金設定。';
        $r['messages'] = $tr['Please select the correct agent level, membership level, bouns level, commission set'];
      }
    ...
    if($r['code'] == 1) {
      ...
      echo $logger;
	 	...
	 	// 請填入資料
    // $tr['Please fill in the * field data'] = '請填入 * 欄位的資料';
	   echo '<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span>'.$tr['Please fill in the * field data'].'</div>';
	}


// ----------------------------------------------------------------------------
} elseif($action == 'member_check' AND isset($_SESSION['agent']) AND $_SESSION['agent']->therole == 'R' ) {
  // ------------------
  // 檢查 會員帳號欄位
  // ------------------
  if(isset($_POST['account_create_input']) AND $_POST['account_create_input'] != NULL ) {
    $memberaccount_create_check_return = memberaccount_create_check($_POST['account_create_input']);
